...
function HeaderImage() {
  ...
  return (
    <>
      {/* Mobile: show full image without mask to avoid cropping */}
      <div className={clsx('md:hidden')}>
        <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <Image
            alt="Zakariya Younis Al-Shuaibi Illustration"
            src="/assets/images/me.png"
            width={360}
            height={414}
            className={clsx('mx-auto h-auto w-[320px] max-w-none', 'dark:brightness-[.82]')}
            quality={100}
            priority
          />
        </m.div>
      ...
      {/* Desktop: masked decorative shape */}
      <div
        className={clsx('relative hidden md:block', 'md:h-[480px] md:w-[480px]', 'lg:h-[590px] lg:w-[603px]')}
        style={{
      ...
      >
        ...
        >
          <div className={clsx('absolute bottom-0 right-0 overflow-hidden')}>
            <m.div className={clsx('absolute z-[10]')} initial={{ opacity: 1 }} animate={controlsHeaderOutline}>
              <HeaderImageAnimation
                onAnimationComplete={() => {
                  ...
                  controlsHeaderImage.start({ opacity: 1, transition: { duration: 0.15 } });
                }}
              />
            </m.div>
